...
def main(args):
    parser = argparse.ArgumentParser(description='Run whole pipeline (copy, convert, organize) by default')

    #in
    parser.add_argument('srcFolderPath', action="store", help="Sources folder (PATH/____beavoda/")
    parser.add_argument('-destFolderPath', action="store", default=".", help="Dest path (all stages folders will be here). By default: '.'")

    #copy stage
    parser.add_argument('--skipCopy', action="store_true", default=False, help="skip copy stage")
    parser.add_argument('-copyPathPrefix', action="store", default="____beavoda", help="Paths at mapping file has ____beavoda/ prefix (4.10.2017)")
    parser.add_argument('-copyPatterns', action="append", default=['*.doc', '*.docx'], help="Patterns for source docs copy stage filtering (default *.doc, *.docx)")

    #organize stage
    parser.add_argument('--skipOrganize', action="store_true", default=False, help="skip organize stage")
    parser.add_argument('-mappingFilePath', action="store", default="./mapping.xlsx", help="Path to Roza's maping file. Default './mapping.xlsx'")
    parser.add_argument('-organizePatterns', action="append", default=['*.doc', '*.docx'], help="Patterns to files filter (Default: *.docx and *.doc)")
    parser.add_argument('-langMapFilePath', action="store", default="./langMap.json", help="Map lang3letters -> lang2letters")

    #converting stage
    parser.add_argument('--skipConvert', action="store_true", default=False, help="skip convert stage")
    parser.add_argument('-postgresqlOptFilePath', action="store", default="./postgresqlOpt.json", help="For Mdb queries")
    parser.add_argument('-tidyOptionsFilePath', action="store", default="./tidyOptions.json", help="for tidy (html cleaner)")
